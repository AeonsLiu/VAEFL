...
from models import Extractor, Classifier, Generator
...
class Client():
    ...
    def init_net(self):
        ...
        self.net["generator"] = Generator()  # G
        self.frozen_net(["extractor", "classifier", "generator"], True)
    ...
    def local_train(self, current_round):
        ...
        for epoch in range(args.local_epoch):
            ...
            for batch, (x, y) in enumerate(self.trainloader):
                ...
                GC = self.net["classifier"](G)
                GC_loss = self.CE_criterion(GC, y)

                (EC_loss + GC_loss).backward()
                self.EC_optimizer.step()
                EC_loss_meter.update(EC_loss.item())
                GC_loss_meter.update(GC_loss.item())

            self.frozen_net(["extractor", "classifier"], True)

            EC_loss = EC_loss_meter.get()
            GC_loss = GC_loss_meter.get()
            EC_acc = self.local_val()
            logger.info("Client:[%2d], Epoch:[%2d], EC_loss:%2.6f, GC_loss:%2.6f, EC_acc:%2.6f" % (
                self.id, epoch, EC_loss, GC_loss, EC_acc))
