...
class Domainnet:
    ...
    def data_partition(self):
        ...
        if args.algorithm == "feddf":
            server_domain = "painting"
            dataset = ImageFolder(os.path.join(args.data_dir, "domainnet-4", server_domain), train_transform)
            size = len(dataset)
            index = np.arange(size)
            np.random.shuffle(index)
            self.server_dataset = Subset(dataset, index[:32000])
            return self.trainsets, self.valsets, self.testsets, self.server_dataset
        else:
            return self.trainsets, self.valsets, self.testsets
