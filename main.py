...
if args.algorithm == 'local':
    ...
    from clients.fedvae import Client
    from servers.fedvae import Server
...
def main():
    ...
    while True:
        ...
        if early_stop:
            if args.algorithm == 'fedavg':
                for i in range(args.n_clients):
                    clients[i].reconstruction(args.dataset)
            break
        ...
        if args.algorithm == 'fedavg' and current_round == 0:
            for i in range(args.n_clients):
                clients[i].reconstruction(args.dataset)
    ...
    b = datetime.now()
    print("training time: %d" % (b - a).seconds)
    for i in range(args.n_clients):
        logger.info("client:%2d, test acc:%2.6f, best epoch:%2d" % (i, test_accs[i], best_rounds[i]))
